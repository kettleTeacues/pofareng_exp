...
def test_get_lifelog():
    ...
    assert len(res.json()) == len(dummy_records['lifelog']) + 2
    assert res.json()[len(dummy_records['lifelog']) + 0]['lifelog']['event'] == 'sleep2_updated'
    assert res.json()[len(dummy_records['lifelog']) + 1]['lifelog']['event'] == 'sleep'